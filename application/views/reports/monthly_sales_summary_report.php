...
<script>
    var star_date = "<?= (isset($form_data['start_date']) ? $form_data['start_date'] : ''); ?>";
    console.log(star_date);
</script>


<section class="content" style="padding: 10px 10px 0px 10px;">
    <div class="card card-default color-palette-bo">
        <div class="card-header">
            <div class="d-inline-block">
                <h3 class="card-title"> <i class="fas fa-bottle"></i>
                    Monthly Sales Summary</h3>
            </div>
        ...
        <?php echo form_open_multipart(base_url('reports/Sales_Report/monthly_sales_report'), 'class="form-horizontal"');  ?>
        <div class="card-body">
            <div class="row">
                ...
                <!-- <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="end_date" id="end_date" class="form-control" placeholder="Select End  Time" value="<?php // (isset($form_data['end_date']) ? $form_data['end_date'] : ''); 
                                                                                                                                    ?>" autocomplete="off">
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        <div class="card-footer mt-4 card-footer bg-transparent p-0 d-flex justify-content-end">
                            ...
                            <button class="btn btn-warning mr-2" onclick="window.location=<?= base_url('reports/Sales_Report/monthly_sales_report') ?>"><i class="fa fa-eraser mr-2"></i>Reset</button>
    ...
    <div style="margin-left: 10px; margin-right: 10px; margin-bottom: 20px; " class="card" id="temp_voilation">
        ...
        <div class="card-body table-responsive">
            <table id="na_datatable" class="table datatable  table-bordered datatable-striped" width="100%">
                ...
                <thead class="m0 mb11" style="white-space:nowrap">
                    <tr>
                    ...
                    <tr>
                        <th colspan="11">Monthly Sales Summary Report For:-
                ...
                <tbody>
                    ...
                    <?php $i = 0;
                    $liquor_opening_qty = intval(0.00);
                    $sale_qty = intval(0.00);
                    $cost_value = floatval(0.00);
                    $sale_value = floatval(0.00);
                    $close_cost_value = floatval(0.00);
                    $liquor_balance_qty = intval(0.00);
                    $profit = floatval(0.00);
                    ?>
                    ...
                    <?php
                        $liquor_opening_qty = $liquor_opening_qty + floatval($row['liquor_opening_qty']);
                        $sale_qty = $sale_qty + intval($row['liquor_sale_qty']);
                        $liquor_balance_qty = $liquor_balance_qty + floatval($row['liquor_balance']);
                        $cost_value = $cost_value + floatval($row['liquor_total_purchase_price']);
                        $sale_value = $sale_value + floatval($row['liquor_total_sale_price']);
                        $close_cost_value = $close_cost_value + floatval($row['closing_stock_value']);
                        $profit = $profit + floatval($row['liquor_profit']);
                    } ?>
                    <tr>
                        ...
                        <td></td>
                        <td></td>
                        <td><?php //$liquor_opening_qty; 
                            ?></td>
                        <td><?= $sale_qty; ?></td>
                        <td><?= $cost_value ?></td>
                        <td><?= $sale_value ?></td>
                        <td><?= $profit ?></td>
                        <td><?= $liquor_balance_qty ?></td>
                        <td><?= $close_cost_value ?></td>
                    </tr>
                    <!-- <tr>
                <td colspan="1"></td>
                    <td colspan="1"></td>
