...
class RBAC
{
	...
	{
		$module = $this->obj->uri->segment(1) . "/" . $this->obj->uri->segment(2);
		// if($this->obj->is_supper){
		return 1;
		// }
		// elseif(!$this->check_module_permission($module)) //sending controller name
		// {
	...
	function check_module_permission($module) // $module is controller name
	{
		...
		elseif (isset($this->obj->module_access[$module])) {
			...
			if ($access)
				...
				return 1; //reverted before going to production
		}
	}
	...
	{
		$module = $this->obj->uri->segment(1) . "/" . $this->obj->uri->segment(2);
		if (isset($this->obj->module_access[$module][$operation]))
			return 1;
		else
			return 1; //reverted before going to production
	}
	...
	function checkSessionData() {
		$CI = & get_instance();
		$user_logged_in = $CI->session->userdata('admin_id');
		if ($user_logged_in === FALSE || !isset($_SESSION["admin_id"])) {
