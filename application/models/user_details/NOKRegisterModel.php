 ...
 * @author Jitendra Pal
 */
class NOKRegisterModel extends CI_Model
{
    ...
    public function fetchRetireeDetails($irlano)
    {
        ...
        // $query = "SELECT nok_name,relationship_type, bh.id,concat(bh.irla,' - ', bh.rank,'. ',bh.name) AS name 
        // FROM bsf_hrms_data bh 
        // INNER JOIN nok_details nd ON nd.hrms_id=bh.id 
        // WHERE bh.irla like '%$irlano%'  AND bh.rank is NOT NULL";
    ...
    public function checkRetireeData($data)
    {
        ...
        $db = $this->db;
        // print_r($data);
        // $query = "SELECT bh.name, bh.irla, bh.mobile_no, bh.adhaar_card, bh.date_of_birth, bh.rank, bh.email_id, bh.posting_unit, bh.capf_force, bh.ppo_photo, bh.card_photo, bh.signed_photo, bh.user_photo, bh.status, nd.nok_name, nd.is_active, nd.relationship_type FROM bsf_hrms_data bh
        //             INNER JOIN nok_details nd ON nd.hrms_id=bh.id
        //             WHERE bh.id=$data->perssonel_no";
        ...
        if ($result[0]->user_count == 0) {
            if ($user_result[0]->status == "RETIRED") {
                ...
                // $result_array['message'] = "The given perssonel No- $username data has been sent for verification";
    ...
    public function addNOKData($data)
    {
        $db = $this->db;
        $query = "CALL SP_ADD_UPDATE_NOK_DATA('$data')";
        // print_r($query);
        // die;
        $response = $db->query($query);
        $result = $response->result();
    ...
    public function fetchInitialFormDetails()
    {
        ...
        $data['relationship_select_option_array'] = $this->fetchRelationship($db);
    ...
    public function fetchRelationship($db)
    {
        $query = "select id,relationship_type from nok_relationship";
        $response = $db->query($query);
        $result = $response->result();
        // print_r($result);
        return $result;
