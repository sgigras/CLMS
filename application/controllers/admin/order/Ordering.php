...
class Ordering extends MY_Controller
{
    ...
    public function continueCartShoppingSession()
    {
        if ($this->session->userdata('continue_cart_shopping_session')) {
            ...
            $cart_type = $cart_details['cart_type'];
            $state_record = $this->stockist_order_model->fetchCanteen($cart_type, 'your_order');

            $displayData = array_merge($cart_details, $state_record);
            if ($cart_details['page_mode'] == 'delivery_cart') {
                $displayData['redirection_mode'] = 'delivery_cart_page';
            } else {
    ...
    public function displayProducts()
    {
        ...
        if (count($result) > 0) {
            ...
            $message_html = '<h2 style="color:red">Liquor Not available</h2>';
            echo $message_html;
        }
